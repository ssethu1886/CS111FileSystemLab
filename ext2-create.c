...
void write_block_bitmap(int fd) {
	...
	for (int i = 0 ;i < 1024; i++) {
	   ...
	   if (i < 2)  {
	      block_bitmap[i] = 0xff; //1st - 16th bit set to 1 (used)
           } else if ( i == 2) {
	      block_bitmap[i] = 0x7f; //24th bit set to 0 (free), 17th to 23rd set to 1 (used)
	   } else if (i >=3 && i < 127) {
	      block_bitmap[i] = 0x00; //25th bit to 1016th bit set to 0 (free)
           } else if (i == 127) { 
              block_bitmap[i] = 0x80; //1024th bit set to 1 (unavailable), 1017-1023 set to 0 (free)
	   } else if ( i > 127)  {
	      block_bitmap[i] = 0xff; //1025th bit to 8192th bit set to 1 (unavailable)
           }
	}	 
...
void write_inode_bitmap(int fd) {
	u8 inode_bitmap[1024]; 
	for (int i = 0;i < 1024; i++) {

	   if (i == 0)  {
	      inode_bitmap[i] = 0xff; //1st - 8th bit set to 1 (unavailable)
           } else if ( i == 1) {
	      inode_bitmap[i] = 0x1f; //9th-13th bit set to 1(9 and 10 unavail, 11-13 used),14th-16th bit set to 0(free)
	   } else if (i > 1 && i < 16) {
	      inode_bitmap[i] = 0x00; //17th bit to 128th bit set to 0 (free)
	   } else if (i >= 16 ) {
	     inode_bitmap[i] = 0xff; 
           }
...
void write_inode_table(int fd) {
	...
	hello_world_file_inode.i_size = 12;
	...
	hello_world_file_inode.i_blocks = 2; 
	hello_world_file_inode.i_block[0] = HELLO_WORLD_FILE_BLOCKNO;
	write_inode(fd, HELLO_WORLD_INO, &hello_world_file_inode);

	struct ext2_inode hello_link_inode = {0};
	hello_link_inode.i_mode = EXT2_S_IFLNK
	                              | EXT2_S_IRUSR
	                              | EXT2_S_IWUSR
	                              | EXT2_S_IRGRP
	                              | EXT2_S_IROTH;
	hello_link_inode.i_uid = 1000;
	hello_link_inode.i_size = 11;
	hello_link_inode.i_atime = current_time;
	hello_link_inode.i_ctime = current_time;
	hello_link_inode.i_mtime = current_time;
	hello_link_inode.i_dtime = 0;
	hello_link_inode.i_gid = 1000;
	hello_link_inode.i_links_count = 1;
	hello_link_inode.i_blocks = 0;   
	memcpy(hello_link_inode.i_block,"hello-world",11) ;
	write_inode(fd, HELLO_INO, &hello_link_inode); 
}
